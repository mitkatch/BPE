...
|| SOURCE:              $Source: /export/home/tomcat/Build/espeedfeedII/espeedII/RCS/ESCFETIProcessor.cpp,v $
|| STATE:               $State: Exp $
|| ID:                  $Id: ESCFETIProcessor.cpp,v 1.3 2006/03/30 21:54:41 tomcat Exp $
|| REVISION:    $Revision: 1.3 $
|| LOG:                 $Log: ESCFETIProcessor.cpp,v $
|| LOG:                 Revision 1.3  2006/03/30 21:54:41  tomcat
|| LOG:                 handle Prov price normilized
|| LOG:
|| LOG:                 Revision 1.2  2006/03/28 20:43:32  mikhailt
|| LOG:                 commented some output
|| LOG:
				...
				CFETI_CMDDATA cmdData, CFETI_UD userData)
{
	...
	if(cmdStatus && cmdStatus->statusText){
		Logger::getLogger()->log(Logger::TYPE_INFO, "ESCFETIProcessor::SystemCallback() status %s, cmd %d", cmdStatus->statusText, cmd);
	}
	...
	switch(cmd){
			...
			if (ESCFETISession::currentSession){
				delete [] ESCFETISession::currentSession;

			}
			//ESCFETISession::isQueryDone = true;
			//ESCFETISession::numInstruments = 0;	
			ESCFETISession::currentSession = strdup(l->sessionId);
			for(ts = l->ts; ts && *ts; ts++) {
				if ((*ts)->tsId == ESCFETISession::room) {
					result = CFETIConnect(ESCFETISession::currentSession, ESCFETISession::_password, (*ts)->tsId, 0, ConnectCallback, 0, &tradeSettings);
					break;
				}
			}
			switch(result) {
		...
		case CFETI_LOGOUT_REJECTED:
		{
			...
			break;
		default:
			Logger::getLogger()->log(Logger::TYPE_INFO, "ESCFETIProcessor::SystemCallback() Logout rejected <default>: session %s, %d", cmdData ? (char *)cmdData : "unknown", cmd);
			break;
	}
}

void
ESCFETIProcessor::ConnectCallback(CFETI_CMD cmd, CFETI_CMD_STATUS cmdStatus, CFETI_CMDDATA cmdData, CFETI_UD userData)
{
	...
	case CFETI_CONNECTION_ACCEPTED: 
		{
			Logger::getLogger()->log(Logger::TYPE_INFO,"ESCFETIProcessor::ConnectCallback CFETI_CONNECTION_ACCEPTED");
			c = (CFETI_CONNECT_INFO *)cmdData;
			//if (ESCFETISession::isQueryDone == true)
			//	ESCFETISession::isQueryDone = false;
			...
			if (ESCFETISession::currentTradingSession == 0) {
				...
				if (result != CFETI_SUCCESS) {
					Logger::getLogger()->log(Logger::TYPE_ERROR, "CFETIPostMessage (QUERY) Failed : Error %d (%s)", result, CFETIGetLastError()); 
				}

			}else {
			//	Logger::getLogger()->log(Logger::TYPE_INFO, "Reset ESCFETISession::isQueryDone for new %d, old %d", c->sessionId, ESCFETISession::currentTradingSession ); 
			//	//if (ESCFETISession::currentTradingSession != c->sessionId) {
				ESCFETISession::currentTradingSession = c->sessionId;
				ESCFETISession::isQueryDone = true;
			//	//}
			}
	...
	case CFETI_CONNECTION_REJECTED:
		Logger::getLogger()->log(Logger::TYPE_ERROR, "Connection Rejected for session %s", (char *)cmdData);
		break;
	case CFETI_CONNECTION_TERMINATED:
		c = (CFETI_CONNECT_INFO *)cmdData;
		Logger::getLogger()->log(Logger::TYPE_INFO,"Connection to %d terminated - trading session id %d", c->tradingSystem.tsId, c->sessionId);
		break;
	case CFETI_DISCONNECT_ACCEPTED:
		c = (CFETI_CONNECT_INFO *)cmdData;
		Logger::getLogger()->log(Logger::TYPE_INFO,"Disconnect from %d accepted - trading session id %d", c->tradingSystem.tsId, c->sessionId);
		break;
	case CFETI_DISCONNECT_REJECTED:
		c = (CFETI_CONNECT_INFO *)cmdData;
		Logger::getLogger()->log(Logger::TYPE_ERROR,"Disconnect from %d rejected - trading session id %d", c->tradingSystem.tsId, c->sessionId);
		break;
	case CFETI_QUERY_ACCEPTED:
		{
			...
			switch(qr->query.queryType) {
				...
				case CFETQ_INSTRUMENT: {
					...
					ESCFETISession::instArray = (char**)realloc(ESCFETISession::instArray, ESCFETISession::numInstruments * sizeof(char*));
					...
					for (kdx = 0; kdx < qr->result.instrument.numInstruments; kdx++) {
						Logger::getLogger()->log(Logger::TYPE_INFO, "CFETQ_INSTRUMENT \t%s : %s", 
							...
							qr->result.instrument.instrumentList[kdx].description? qr->result.instrument.instrumentList[kdx].description : "(no description)");
						int index  = prevNumInstrument + kdx;
						char* inst =const_cast<char*>(qr->result.instrument.instrumentList[kdx].name);
		...
		case CFETI_SUBSCRIBE_ACCEPTED:
			_subscribedInst++;
			Logger::getLogger()->log(Logger::TYPE_INFO, "Subscribe Accepted : %s [%d == %d]", ((CFETI_INSTRUMENT_DATA)cmdData)->instName, _subscribedInst, ESCFETISession::numInstruments);
		...
		case CFETI_SUBSCRIBE_REJECTED:
			Logger::getLogger()->log(Logger::TYPE_ERROR, "Subscribe Rejected : \"%s\"", (CFETI_INSTRUMENT)cmdData);
			break;
		case CFETI_UPDATE:
			OutputInstrumentData((CFETI_INSTRUMENT_DATA)cmdData);
			break;
		case CFETI_INST_QUERY_COMPLETE:
			Logger::getLogger()->log(Logger::TYPE_INFO, "ESCFETIProcessor::ConnectCallback CFETI_INST_QUERY_COMPLETE");
			ESCFETISession::isQueryDone = true;
			break;
		default:
			break;
...
{
	bool updated = false;
	Logger::getLogger()->log(Logger::TYPE_INFO, "ESCFETIProcessor::OutputInstrumentData Market Data Update : %s\n", instData->instName);
	unsigned short property = 0;
	int idx;
	ESInstrument *esI = _myProcessor->getESInstrument(instData->instName);
	if (esI == NULL){
		Logger::getLogger()->log(Logger::TYPE_ERROR, "ESCFETIProcessor::OutputInstrumentData failed to getESInstrument for update %s\n", instData->instName);
		return ;
	}
	...
	for (idx = 0; idx < instData->numFields; idx++){
		...
		switch(instData->fieldTable[idx].fieldType)
		{
			case CFETI_FIELDTYPE_INT8:
				//Logger::getLogger()->log(Logger::TYPE_INFO, "No handler Field 0x%4.4x : %d", instData->fieldTable[idx].fieldId, instData->fieldTable[idx].fieldValue.int8);
				break;
			case CFETI_FIELDTYPE_BYTE:
				//Logger::getLogger()->log(Logger::TYPE_INFO, "No handler Field 0x%4.4x : %u", instData->fieldTable[idx].fieldId, instData->fieldTable[idx].fieldValue.byte);
				break;
			case CFETI_FIELDTYPE_UINT16:
			{
				//fprintf(stdout, "Field 0x%4.4x : %u\n", instData->fieldTable[idx].fieldId, instData->fieldTable[idx].fieldValue.int16);
			...
			{
				//fprintf(stdout, "Field 0x%4.4x : %d\n", instData->fieldTable[idx].fieldId, instData->fieldTable[idx].fieldValue.uint16);
				int iVal = instData->fieldTable[idx].fieldValue.uint16;
				esI->setValByIndex(iVal, elemIndex);	
			}
				break;
			...
			{
				//fprintf(stdout, "Field 0x%4.4x : %d\n", instData->fieldTable[idx].fieldId, instData->fieldTable[idx].fieldValue.int32);
				int iVal = instData->fieldTable[idx].fieldValue.int32;
				esI->setValByIndex(iVal, elemIndex);	
			}
				break;
			case CFETI_FIELDTYPE_UINT32:
				//Logger::getLogger()->log(Logger::TYPE_INFO, "No handler Field 0x%4.4x : %u", instData->fieldTable[idx].fieldId, instData->fieldTable[idx].fieldValue.uint32);
				break;
			case CFETI_FIELDTYPE_DATETIME:
			{
				//fprintf(stdout, "Field 0x%4.4x : %24s\n", instData->fieldTable[idx].fieldId, ctime(&instData->fieldTable[idx].fieldValue.dateTime));
			...
			{
				//fprintf(stdout, "Field 0x%4.4x : %s\n", instData->fieldTable[idx].fieldId, instData->fieldTable[idx].fieldValue.string);
			...
			case CFETI_FIELDTYPE_BUFFER:
				if (instData->fieldTable[idx].fieldId == CFETF_BID_LIST_1){
					//Logger::getLogger()->log(Logger::TYPE_INFO, "No handler Field 0x%4.4x : (Buffer, length %d)", instData->fieldTable[idx].fieldId, instData->fieldTable[idx].fieldValue.buffer.len);
				}
				{
					CFETI_PARTICIPANT_LIST pList = NULL;
					CFETI_RC rc = CFETIDecodeFieldExt(ESCFETISession::currentSession, ESCFETISession::currentTradingSession, &instData->fieldTable[idx], (void*)&property, (void **) &pList);
					if (rc == CFETI_SUCCESS){
						for (unsigned int jdx = 0; jdx < pList->numParticipants; jdx++)
							;//Logger::getLogger()->log(Logger::TYPE_INFO, "No handler \t%d: %d %f %d %d", jdx, pList->participant[jdx].tsId, pList->participant[jdx].size, pList->participant[jdx].code, pList->participant[jdx].counterpartyState);
					} else {
						Logger::getLogger()->log(Logger::TYPE_ERROR, "\tError %d : %s\n", rc, CFETIGetLastError());
					}
				}
				break;
			case CFETI_FIELDTYPE_DECIMAL:
			{
				//fprintf(stdout, "Field 0x%4.4x : %f\n", instData->fieldTable[idx].fieldId, instData->fieldTable[idx].fieldValue.decimal);
				double dVal = instData->fieldTable[idx].fieldValue.decimal;
				esI->setValByIndex(dVal, elemIndex);
			}
				break;
			default:
				//Logger::getLogger()->log(Logger::TYPE_INFO, "No handler Field 0x%4.4x : (unknown data type)\n", instData->fieldTable[idx].fieldId);
				break;
		}
	}
	if(updated)
