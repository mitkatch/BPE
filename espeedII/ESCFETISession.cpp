...
|| SOURCE:              $Source: /export/home/tomcat/Build/espeedfeedII/espeedII/RCS/ESCFETISession.cpp,v $
|| STATE:               $State: Exp $
|| ID:                  $Id: ESCFETISession.cpp,v 1.1 2005/11/17 21:57:58 mikhailt Exp $
...
ESCFETISession::Run()
{
	...
	while (run_forever){
		...
		for (int i = 0; i < numInstruments; i++) {
			Logger::getLogger()->log(Logger::TYPE_INFO, "ESCFETISession::Run() subscribe  %d [%d] %s", (i +1), numInstruments,  instArray[i]);
			//take a break on 100
			if ((i % 100) == 0) {
				sleep(2);
			}
...
ESCFETISession::shutdown()
{
	...
	if (currentTradingSession != 0) {
		Logger::getLogger()->log(Logger::TYPE_INFO, "Initialing Disconnect from Trading System: %d , thread %d", room, pthread_self());
		result = CFETIDisconnect(currentSession, currentTradingSession, 0);
		if (result != CFETI_SUCCESS) {
			Logger::getLogger()->log(Logger::TYPE_ERROR, "CFETIDisconnect Failed : Error %d (%s)", result, CFETIGetLastError());
		}
	}

	sleep(2);

	Logger::getLogger()->log(Logger::TYPE_INFO, "Initialing a Logout ...%s", (currentSession != NULL?currentSession : "NULL session"));
	result = CFETILogout(currentSession, 0);
	if (result != CFETI_SUCCESS) {
		Logger::getLogger()->log(Logger::TYPE_ERROR, "CFETILogout Failed : Error %d (%s)", result, CFETIGetLastError());
	}
