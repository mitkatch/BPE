...
|| SOURCE:              $Source: /repository/projects/Canpx/libutil/Subscriber.cpp,v $
|| STATE:               $State: Exp $
...
void
Subscriber::setSubscribedFields(const string& address, const string& in, const string& out)
{
	//if user doesn't provide out field it means out = in subscribed field

	map<string, FieldMap>::iterator it = _mapField.find(address);
	if (it != _mapField.end()){
		(*it).second.fields[in] = (out.empty()?in:out);
	} else {	
		//_mapField[address][in]=(out.empty()?in:out);
		FieldMap newFM;
		newFM.fields[in] = (out.empty()?in:out);;
...
void
Subscriber::getMappedField(const string &address, FieldMap &fm)
{
	//string str;
