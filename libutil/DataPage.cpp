/************************************************************************
||
|| DATE:                $Date: 2006/03/23 18:54:54 $
|| SOURCE:              $Source: /repository/projects/Canpx/libutil/DataPage.cpp,v $
|| STATE:               $State: Exp $
|| ID:                  $Id: DataPage.cpp,v 1.7 2006/03/23 18:54:54 nickz Exp $
|| REVISION:    $Revision: 1.7 $
|| LOG:                 $Log: DataPage.cpp,v $
|| LOG:                 Revision 1.7  2006/03/23 18:54:54  nickz
...
bool
DataPage::addPageRow(DataPage::PageRow& pagerow, int row)
{
	bool ret = false;
	pthread_mutex_lock(&m_DataMutex);
	map<int, PageRow>::iterator it = _image.find(row);
...
DataPage::update(int row, int column, const string& data, int video, int bg, int fg, int font)
{
	...
	if (it != _image.end()){
		...
		// Persist data
		persistData( row, column, data, video, bg, fg, font );
...
void
DataPage::persistData( int row, int column, const string& data, int video, int bg, int fg, int font)
{
...
DataPage::notifyListeners(int row1, int row2, int column1, int column2)
{
	...
	for (it; it != _listeners.end(); it++){
		...
		for (int j = 0; j < (*it).second.size(); j++){
			if ( (*it).second[j] >= slot){
				pthread_mutex_unlock(&m_ListenerMutex);
				//cout<<"DataPage::notifyListener "<<j<<" R1:"<<(*it).second[j].row1<<", R2:"<<(*it).second[j].row2<<", C1:"<<(*it).second[j].column1<<", C2:"<<(*it).second[j].column2<<endl;
...
bool
DataPage::update(int row, int column, const string& data, Page& ansiPage)
{
    pthread_mutex_lock(&m_DataMutex);
    bool ret = false;
    map<int, PageRow>::iterator it = _image.find(row);
	...
	for(int i = 0; (pagerow.size() > column + i) && i < data.size(); i++ )
	{
	    // Get Page cell.  Remember, ANSI page is 1-based.
	    PageCell cell = ansiPage.getPageCell(row+1, column+i+1);

	    pagerow[column +i].data = data[i];
	    pagerow[column +i].bg = cell.getBackgroundColor();
	    pagerow[column +i].fg = cell.getForegroundColor();
	    pagerow[column +i].font = cell.getGraphicSet();
	    pagerow[column +i].video = toVideo(cell.getStyle());
		//cout << toVideo(cell.getStyle());
	    ret = true;
	}
	//cout << endl;
	// Persist data
	PageCell cell = ansiPage.getPageCell(row+1, column+1);
	_lastUpdate.row = row; 
	_lastUpdate.column = column;
	_lastUpdate.data = data;
	_lastUpdate.video = cell.getStyle();
	_lastUpdate.bg = cell.getBackgroundColor(); 
	_lastUpdate.fg = cell.getForegroundColor(); 
	_lastUpdate.font = cell.getGraphicSet(); 
	persistData( row, column, data, cell.getStyle(), cell.getBackgroundColor(),
		     cell.getForegroundColor(), cell.getGraphicSet() );

	pthread_mutex_unlock(&m_DataMutex);
	//cout << "DataPage::update about to notifyListeners R:"<<row<<",C1:"<<column<<", C2:"<<column+ data.size() -1<<endl;
	if (data.size())
		notifyListeners(row, row, column, column+ data.size() -1);//begins from zero 
	pthread_mutex_lock(&m_DataMutex);
    }
...
/****
int
DataPage::toVideo(rfa::AnsiPage::PageCell::CellStyle style)
{
	int v = DataPage::V_NONE;
	switch(style) 
	{
		case PageCell::blink:
			v = DataPage::V_BLINK;
			break;
		case PageCell::underline:
			v = DataPage::V_UNDERLINE;
			break;
		case PageCell::reverse:
			v = DataPage::V_HIGHLIGHT;
			break;
	}
...
{
	int v = DataPage::V_NONE;
	if (style & PageCell::blink)
		v |= DataPage::V_BLINK;
	if (style & PageCell::underline)
		v |= DataPage::V_UNDERLINE;
	if (style & PageCell::reverse)
		v |= DataPage::V_HIGHLIGHT;
	if (style & PageCell::bright)
		v |= DataPage::V_HIGHLIGHT;
